...
from ip.utils.data_proc import sample_to_cond_demo, subsample_pcd, save_sample
import torch
    ...
    for i in pbar:
        ...
        for k in range(max_execution_steps):
            ...
            if k == 0 and i == 0:
                ...
                for j in range(min(3, len(actions))):
                    trans = actions[j, :3, 3]
                    mag = np.linalg.norm(trans)
                    print(f"DEBUG: Action[{j}] trans: {trans}, mag: {mag}")
