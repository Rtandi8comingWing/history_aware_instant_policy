...
class ContinuousPseudoDataset(Dataset):
    ...
    def __init__(self, 
        ...
        print(f"Pre-generating {buffer_size} samples...")
        while self.buffer.qsize() < buffer_size // 2:
            time.sleep(0.1)
    ...
    def _generation_worker(self, worker_id):
        ...
        while not self.stop_generation.is_set():
            try:
                ...
                if self.generation_errors % 100 == 0:
                    print(f"Worker {worker_id} generation error count: {self.generation_errors}")
                time.sleep(0.1)
    ...
    def _generate_one_sample(self, generator):
        ...
        # Select context demos (2-4 demos, excluding live)
